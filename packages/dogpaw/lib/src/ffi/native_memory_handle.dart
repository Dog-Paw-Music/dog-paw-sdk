...
class NativeMemoryHandle {
  ...
  NativeMemoryHandle({
    ...
    _bufferSize = _bridge.getDataSize(
        _dataTypeIdx, _indexTypeIdx, _indexDim1, _indexDim2);
    if (_bufferSize > 0) {
  ...
  void initialize({
    ...
    if (_isContinuous) {
      ...
      try {
        if (_isOutput) {
          _handle =
              _bridge.producerCreate(qPtr, sPtr, _dataTypeIdx, _indexTypeIdx);
        } else {
          _handle =
              _bridge.consumerCreate(qPtr, sPtr, _dataTypeIdx, _indexTypeIdx);
        }
      } finally {
        calloc.free(qPtr);
        calloc.free(sPtr);
  ...
  bool adjustBufferSize(int delta) {
    if (!_isOutput || !_isContinuous || _handle == null) {
      AppLogger.debug(
          'adjustBufferSize: Precondition failed - isOutput: $_isOutput, isContinuous: $_isContinuous, handle null: ${_handle == null}');
      return false;
    }

    AppLogger.debug(
        'adjustBufferSize: Before - handle: $_handle, _buffer: $_buffer, _bufferSize: $_bufferSize, delta: $delta');
    ...
    try {
      ...
      if (success) {
        ...
        AppLogger.debug(
            'adjustBufferSize: C++ call succeeded, reallocating Dart buffer from $_bufferSize to $newBufferSize');
        ...
        AppLogger.info(
            'Adjusted buffer size by $delta, new size: $_bufferSize, new buffer: $_buffer');
      } else {
  ...
  bool writeBytes(Uint8List data) {
    ...
    if (data.length != _bufferSize) {
      AppLogger.warning(
          'WARNING: Data size mismatch! data.length=${data.length}, _bufferSize=$_bufferSize, will copy $len bytes');
    }
    ...
    if (_isContinuous) {
      ...
      if (result < 0) {
        ...
        AppLogger.warning(
            'NativeMemoryHandle.write: producerEnqueue failed: $errorMsg');
        return false;
      }
      if (result == 0) {
        AppLogger.warning(
            'NativeMemoryHandle.write: producerEnqueue returned 0, no consumers notified');
      }
  ...
  int _mapDataType(DataType type) {
    switch (type) {
      case DataType.float:
        return DPPBDataType.float;
      case DataType.float2:
        return DPPBDataType.float2;
      case DataType.float3:
        return DPPBDataType.float3;
      case DataType.float4:
        return DPPBDataType.float4;
      case DataType.int_:
        return DPPBDataType.int_;
      case DataType.int2:
        return DPPBDataType.int2;
      case DataType.toggle:
        return DPPBDataType.toggle;
      case DataType.momentary:
        return DPPBDataType.momentary;
      case DataType.enum_:
        return DPPBDataType.enumVal;
      case DataType.color:
        return DPPBDataType.color;
      case DataType.audioStream:
        return DPPBDataType.audioStream;
      case DataType.keyPress:
        return DPPBDataType.keyPress;
      case DataType.nearPress:
        return DPPBDataType.nearPress;
      case DataType.rawSensors:
        return DPPBDataType.rawSensors;
      case DataType.noteControl:
        return DPPBDataType.noteControl;
      case DataType.midiMessage:
        return DPPBDataType.midiMessage;
      case DataType.ledMessage:
        return DPPBDataType.ledMessage;
      case DataType.keyPosition:
        return DPPBDataType.keyPosition;
      case DataType.voiceMessage:
        return DPPBDataType.voiceMessage;
      case DataType.voiceOutputValue:
        return DPPBDataType.voiceOutputValue;
      case DataType.globalOutputValue:
        return DPPBDataType.globalOutputValue;
      case DataType.dppEditorMessage:
        return DPPBDataType.dppEditorMessage;
      case DataType.custom:
        return DPPBDataType.custom;
      case DataType.scopeBuffer:
        return DPPBDataType.scopeBuffer;
      case DataType.theme:
        return DPPBDataType.theme;
      case DataType.scale:
        return DPPBDataType.scale;
    }
  }
  ...
  int _mapIndexType(IndexType type) {
    switch (type) {
      case IndexType.none:
        return DPPBIndexType.none;
      case IndexType.key:
        return DPPBIndexType.key;
      case IndexType.voice:
        return DPPBIndexType.voice;
