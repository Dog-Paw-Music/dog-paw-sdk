...
def resolve_flutter_shared_packages_root(
    ...
    Requirements:
        The active checkout must still include the shared Flutter package
        directories expected by this workflow.

    Guarantees:
        Raises `RuntimeError` with a clear explanation instead of guessing paths
        when the tool has been detached from its owning checkout.
    ...
    missing_packages = [
        ...
        if not (packages_root / package_name).is_dir()
    ]
    if missing_packages:
        raise RuntimeError(
            "Dog Paw shared Flutter packages are missing from the tool checkout "
            f"at {packages_root}: {', '.join(missing_packages)}"
        )
...
def ensure_flutter_dependency_overrides(project_dir: Path) -> bool:
    ...
    Requirements:
        The active Dog Paw tool checkout must contain the shared Flutter package
        roots expected by `resolve_flutter_shared_packages_root()`.

    Guarantees:
        Writes or refreshes `pubspec_overrides.yaml` in `project_dir`.
